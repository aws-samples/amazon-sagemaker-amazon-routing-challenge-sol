...
This code repository contains our solution to the [Amazon Last Mile Routing Research Challenge](https://routingchallenge.mit.edu/), which aims to integrate real-life experience of Amazon drivers into the solution of optimal route planning. This solution is based on our paper **[Learning from Drivers to Tackle the Amazon Last Mile Routing Research Challenge](https://arxiv.org/)**, and can be deployed as an Amazon SageMaker Processing Job. The example below shows the 110-stop route sequences produced by our method on the left (*driver friendly*) and by conventional TSP solvers on the right (*cost-optimal*). While the cost optimal route has less travelling time in theory, it presents a narrow and sharp V turn that connects package stops (blue circles) from and back to the depo (the red circle at the bottom). A route sequence that reverses its course may appear unappealing to some drivers.
