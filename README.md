...
: *Image caption - Blue solid circles are locations of Amazon delivery packages. Red arrows show the direction for each transit. The **actual depo location** is far down below and out of sight. So it is shown as red solid circles for the purpose of visualization. Road networks are overlaid as gray lines and curves. Note that travel time does not include delivery service time at each stop.*
